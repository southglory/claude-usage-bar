...
개인·업무 로그인이나 [cc-switch](https://github.com/southglory/cc-switch) `ccp`/`ccw`
...
> 여기서 *cc-switch* 는 [`southglory/cc-switch`](https://github.com/southglory/cc-switch) 를 가리킵니다 — `CLAUDE_CONFIG_DIR` 격리로 여러 Claude Code **계정**을 실행하는 `ccp`/`ccw`/`ccx` 를 정의한 PowerShell 모듈입니다. 유명한 [`farion1231/cc-switch`](https://github.com/farion1231/cc-switch) 와는 **이름만 같고**, 그쪽은 계정이 아니라 API **공급자/벤더**(Anthropic·Kimi 등)를 전환하는 별개 도구입니다.

> **Windows·macOS·Linux 모두 동작합니다.** 확장 자체에는 OS 의존 코드가 없습니다. `ccp`/`ccw` 래퍼만 Windows/PowerShell 전용이고 — macOS/Linux 에서는 기본 env-주입 모드(`command` 비워둠)를 쓰거나, `command` 에 본인 셸 alias 를 지정하면 됩니다.
